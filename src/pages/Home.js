...
const Home = () => {
  ...
  <div className="flex items-center justify-center space-x-2">
    ...
    <p className="text-md font-bold text-green-500">({discountPercentage}% OFF)</p>
                ...
                <button className="mt-2 bg-green-600 text-white px-3 py-1 rounded hover:bg-green-700 text-sm flex items-center mx-auto shadow-md">
                  Buy Now
                </button>
              </div>
            );
  ...
  <Carousel responsive={responsive} infinite autoPlay autoPlaySpeed={2500}>
    {categories.map((category) => (
      <Link
        to={`/shop?category=${encodeURIComponent(category.name)}`} // ✅ Pass category name in URL
